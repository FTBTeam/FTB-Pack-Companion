...
public interface PCClientConfig {
    ...
    SNBTConfig INTEGRATIONS = CONFIG.getGroup("integrations");
    SNBTConfig JEI = INTEGRATIONS.getGroup("jei");

    BooleanValue JEI_SORT_RECIPES = JEI.getBoolean("sort_recipes", false)
            .comment("When enabled, the recipe tabs in JEI will be sorted based on the order specified in the JEI integration.");

    BooleanValue JEI_RECIPE_NAMES_DEBUG = JEI.getBoolean("recipe_names_debug", false)
