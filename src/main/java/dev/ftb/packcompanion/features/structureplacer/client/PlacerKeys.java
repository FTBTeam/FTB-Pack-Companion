...
public class PlacerKeys {
    public static final KeyMapping ANCHOR_POS_KEY = new KeyMapping("ftbpackcompanion.key.anchor_pos", GLFW.GLFW_KEY_COMMA, PackCompanion.getKeyCategory());
    public static final KeyMapping ROTATE_POS_KEY = new KeyMapping("ftbpackcompanion.key.rotate_pos", GLFW.GLFW_KEY_PERIOD, PackCompanion.getKeyCategory());
    ...
    public static void onInputEvent(InputEvent.Key event) {
        ...
        if (ANCHOR_POS_KEY.consumeClick()) {
            PacketDistributor.sendToServer(AnchorPlacerPacket.INSTANCE);
        } else if (ROTATE_POS_KEY.consumeClick()) {
            PacketDistributor.sendToServer(new RotatePlacerPacket(player.isShiftKeyDown()));
        }
    }
}
