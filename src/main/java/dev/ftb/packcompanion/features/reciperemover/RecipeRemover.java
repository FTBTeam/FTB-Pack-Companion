...
public class RecipeRemover extends Feature.Common {
    ...
    private static final Set<ResourceLocation> EMPTY_SET = Set.of();

    private static Set<ResourceLocation> removedRecipes;
    ...
    private static Set<ResourceLocation> loadRemovedRecipes() {
        ...
        Set<ResourceLocation> removedRecipesSet = new HashSet<>();
        for (Path jsonFile : jsonFiles) {
            try {
                ...
                for (var recipeElement : recipes) {
                    ...
                    var resourceLocation = ResourceLocation.fromNamespaceAndPath(namespace, recipeName);
    ...
    private int reload(CommandContext<CommandSourceStack> context) {
        ...
        context.getSource().sendSuccess(() -> Component.literal("Recipe remover reloaded, run ").append(
                Component.literal("/reload").withStyle(style -> style.withColor(0xFFAA00).withClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/reload")))
